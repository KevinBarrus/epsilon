...
def _evaluation_section(
    ...
    rows = rows or "<tr><td colspan=24>暂无结果</td></tr>"
    failures = "\n".join(
  ...
  <h3>场景结果</h3>
  <table>
    <thead><tr><th>场景</th><th>任务 ID</th><th>来源</th><th>分组</th><th>基线提交</th><th>变更文件</th><th>类型</th><th>状态</th><th>错误类别</th><th>失败阶段</th><th>错误详情</th><th>停止原因</th><th>写后本地命令</th><th>官方 Harness</th><th>耗时</th><th>模型请求</th><th>工具回合</th><th>并行批次</th><th>工具批次耗时</th><th>工具调用</th><th>重试</th><th>压缩</th><th>估算上下文 Token</th><th>服务端实际 Token</th></tr></thead>
    <tbody>{rows}</tbody>
  </table>
  <h3>失败断言</h3>
  <table>
...
def _result_row(result: EvaluationResult) -> str:
    ...
    return (
        ...
        f"<td>{escape(result.local_verification_status or '-')}</td>"
        f"<td>{escape(result.official_harness_status or '-')}</td>"
        f"<td>{result.duration_ms:.2f} ms</td>"
        f"<td>{result.model_requests}</td><td>{result.tool_rounds}</td>"
        f"<td>{result.parallel_tool_batches}</td>"
        f"<td>{result.tool_batch_duration_ms:.2f} ms</td>"
