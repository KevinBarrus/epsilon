...
可以把“宿主工作区挂载到容器”想成你和容器共同使用一个储物柜：
...
容器里的程序以管理员身份运行，在柜子里创建了 Python 缓存文件 `.pyc`。因为 WSL/Docker 会转换容器身份，这个文件和它所在的 `__pycache__` 小抽屉回到宿主侧后，签收人显示为 `nobody`，而不是当前宿主用户。

评测结束时，宿主程序需要清空这个小抽屉，避免把 Python 缓存误做成代码补丁。但小抽屉的权限是 `755`：当前宿主用户可以打开和阅读，却没有在其中删除文件的权限。就像你能打开公共储物柜查看东西，却无权清空一个登记在别人名下、没有给你写入权限的小抽屉。
...
> 容器 root 写入宿主挂载目录后，经用户命名空间映射生成了宿主用户无权清理的缓存目录；补丁生成必须先清理缓存，因此抛出 `PermissionError` 并中断评测。
