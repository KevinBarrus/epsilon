...
- Scout 默认不设固定工具轮次上限和总运行时限；每次模型请求仍使用客户端网络超时，父请求取消时同步取消仍在运行的 Scout；
- Scout 失败或被父请求取消时，`spawn_agent` 返回结构化错误或向上传播取消，父 Agent 可以继续处理任务。
...
- 子 Agent 失败时返回结构化错误，父 Agent 可以继续处理；
