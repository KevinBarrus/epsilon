...
提交 `735494b` 后重新运行 on 档真机冒烟，结果文件为 `evaluation-results/subagent-smoke-independent-search-fix.jsonl`（原始 JSONL 按仓库规则忽略，不改写历史结果）。本次任务完成，且未再出现工作区全量 Python 扫描导致的长时间卡死。

| 指标 | 上一轮 on | 搜索修复后 on | 变化 |
| --- | ---: | ---: | ---: |
| 任务完成 | 是 | 是 | 不变 |
| 父 Agent actual tokens | 189,743 | **131,030** | **减少 31.0%** |
| Scout actual tokens | 326,894 | **277,156** | **减少 15.2%** |
| 全部 Agent actual tokens | 516,637 | **408,186** | **减少 21.0%** |
| 总耗时 | 839.9 秒 | **269.7 秒** | **减少 67.9%** |
| Scout 调用 | 4 | 4 | 不变 |
| Scout 批次 | 1 个 parallel 批次 | **1 个 parallel 批次** | 不变 |
| context | 未记录 | **4 次，共 699 字符** | 已观察到传递 |
| Scout 结果 | 2 completed / 2 tool_limit | **3 tool_limit / 1 timeout** | 完成质量仍需优化 |
| 缺失 usage | 0 | **1（cancelled）** | 新增 1 个缺失请求 |

本次总 token 相比上一轮减少 21.0%，但仍高于 off 档基线，不能据此宣称多 Agent 已经节省总 token。耗时从 14.0 分钟降到 4.5 分钟，说明此前主要问题确实来自工作区搜索开销，而不是 Scout 串行；本次仍是同一并行批次。

需要注意：4 个 Scout 虽然被并行调度，但 3 个达到 8 轮工具上限、1 个超时，父 Agent 最终自行读取四个文件完成任务。下一步应单独处理 Scout 的工具轮次/超时和取消 usage 记录，不能把这次任务完成误认为 Scout 调查全部成功。
