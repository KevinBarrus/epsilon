...
在 `WriteVerificationPolicy` 内部增加一个纯函数，基于 `run_command` 的 `command` 参数识别常见检查命令。只有这一类命令的结果才会影响写后验证状态；所有写后命令仍保留在轨迹中，便于排查。
...
不调用模型判断命令意图，也不新增可配置规则引擎。第一版使用集中维护的命令标记表：规则透明、可单元测试，新增一种常见检查命令只需在一处增加标记。
