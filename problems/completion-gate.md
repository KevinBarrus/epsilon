...
**门在本次配置下没有起到作用。** 三次门后重跑的 A 档，**每一次的拒绝原因都是
`inconclusive`**（验证器 token 预算耗尽），**没有一次是实质判定**；覆盖率没有提升
（门前 4/40 → 门后 2~4/40），而每次声明完成都要多付验证器的预算。
...
- **覆盖率没有提升**：门前 4/40，门后是 3/4/2 —— 差异在运行间噪声范围内（我们在 oncall 实验里
  标定的运行间波动就有 1.7~1.9×），**不能说是提升，也不能说是下降**；
- **门的判定全是 inconclusive**：三次运行的 `rejected_not_met = 0`、`rejected_inconclusive ≥ 2`，
  未满足项统一是"验证器 token 预算耗尽"；
...
- B/C 在发现"验证器预算问题"后**主动中止**（B 已消耗 8,608,645 token），未纳入结论；
- 三次门后 A 档共消耗约 **23.2M token**（15.8M + 3.8M + 3.5M），smoke 约 0.05M；
- **本轮没有产出可用的 B/C 对照**，因此"门能否提高覆盖率"**没有答案**。
...
- 门后三次 A 档与中止的 B：`evaluation-results/big-repo-read-gate/`
  （`attempt1a` / `attempt1b` / `attempt2` / `attempt3`）
